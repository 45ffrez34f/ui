...
-- Library init
    ...
    local fonts = {}; do
        ...
        local Medium = Register_Font("Medium", 200, "Normal", {
            Id = "Medium.ttf",
            Font = game:HttpGet("https://github.com/45ffrez34f/ui/raw/refs/heads/main/Inter_28pt-Medium.ttf"),
        })

        local SemiBold = Register_Font("SemiBold", 200, "Normal", {
            Id = "SemiBold.ttf",
            Font = game:HttpGet("https://github.com/45ffrez34f/ui/raw/refs/heads/main/Inter_28pt-SemiBold.ttf"),
        })
...
-- Library functions 
    -- Misc functions
        ...
        function library:start_auto_update()
            if library.auto_update_connection then
                library.auto_update_connection:Disconnect()
                library.auto_update_connection = nil
            end
        ...
        function library:stop_auto_update()
            if library.auto_update_connection then
                library.auto_update_connection:Disconnect()
                library.auto_update_connection = nil
            end
            library._last_update = nil
        end
        ...
        function library:get_game_id()
            return tostring(game.GameId or game.PlaceId or "unknown")
        end

        function library:save_config_with_gameid(config_name)
            ...
            if isfile(file_path) then
                ...
                if data.game_id and data.game_id ~= current_game_id then
                    notifications:create_notification({
                        name = "Configs", 
                        info = "Config already exists for another game!\nConfig GameID: " .. data.game_id
                    })
                    return false
                end
            end
            ...
            writefile(file_path, http_service:JSONEncode(config_data))
            library:update_config_list()
            notifications:create_notification({name = "Configs", info = "Saved config:\n" .. config_name})
            return true
        end

        function library:load_config_with_gameid(config_name)
            if not config_name or config_name == "" then
            ...
            if data.game_id then
                ...
                if data.game_id ~= current_game_id then
                    notifications:create_notification({
                        name = "Configs", 
                        info = "Config is for another game!"
                    })
                    return false
                end
            end
            ...
            if data.config then
                library:load_config(data.config)
            else
                library:load_config(raw_data)
            end
        ...
        function library:delete_config_with_gameid(config_name)
            if not config_name or config_name == "" then
                notifications:create_notification({name = "Configs", info = "Please select a config!"})
                return false
            end
    ...
    -- Library element functions
        function library:window(properties)
            local cfg = { 
                ...
                game_name = properties.gameInfo or properties.game_info or properties.GameInfo or "ironclad ui";
            ...
            local items = cfg.items; do
                ...
                library[ "title_frame" ] = items[ "title" ]
                ...
                items[ "other_info" ] = library:create( "TextLabel" , {
                    ...
                    TextColor3 = themes.preset.accent;
                    BorderColor3 = rgb(0, 0, 0);
                    Text = '<font color="rgb(72, 72, 73)">' .. executor_name .. ', </font>' .. cfg.name .. cfg.suffix;
                    Size = dim2(1, 0, 0, 0);
                    Position = dim2(0, -10, 0.5, -1);
                    AnchorPoint = vec2(0, 0.5);
                    BorderSizePixel = 0;
    ...
    local other_section2 = other_column2:section({name = "Other", size = 1, default = true, icon = "rbxassetid://6034509993"})

    local STICKER_DIR = library.directory .. "/stickers"

    local STICKERS = {
        Pinky = {
            url  = "https://raw.githubusercontent.com/45ffrez34f/ui/main/8a27e6d9961f1e1d3ebd5e1fd71e-8848572e39e26a4e310236285d0b.png",
            file = STICKER_DIR .. "/ironclad_pinky.png",
            asset = nil,
        },
        KisKis = {
            url  = "https://raw.githubusercontent.com/45ffrez34f/ui/main/c977b6349631a583963799ce03a8-26f483b89180df066d97e20c5ae6.png",
            file = STICKER_DIR .. "/ironclad_kiskis.png",
            asset = nil,
        },
    }

    local function load_sticker(sticker)
        if isfile and isfile(sticker.file) then
            local ok, asset = pcall(getcustomasset, sticker.file)
        ...
        local ok, body = pcall(function()
            return game:HttpGet(sticker.url, true)
        end)
        if ok and type(body) == "string" and #body > 64 then
            pcall(writefile, sticker.file, body)
            local ok2, asset = pcall(getcustomasset, sticker.file)
        ...
        sticker.asset = sticker.url
    end

    for _, sticker in STICKERS do
        load_sticker(sticker)
    end

    local title = library["title_frame"]

    local sticker_container = library:create("Frame", {
        Parent              = title;
        Name                = "ironclad";
        BackgroundTransparency = 1;
    ...
    library["sticker_container"] = sticker_container

    local sticker_frame = library:create("ImageLabel", {
        Parent              = sticker_container;
        Name                = "Sticker";
        BackgroundTransparency = 1;
        Size                = dim2(1, 0, 1, 0);
        BorderSizePixel     = 0;
        Image               = STICKERS.Pinky.asset or "";
    ...
    library["sticker"] = sticker_frame

    task.spawn(function()
        pcall(function()
            game:GetService("ContentProvider"):PreloadAsync({ sticker_frame })
        end)
    end)

    other_section2:toggle({
        name = "Show Sticker",
        flag = "ui_show_sticker",
        default = false,
        callback = function(value)
            sticker_container.Visible = value
        end,
        seperator = false,
    })
    ...
    other_section2:dropdown({
        name = "Sticker (Choice)",
        flag = "ui_sticker_choice",
        items = {"Pinky", "KisKis"},
        default = "Pinky",
        info = "Choose which sticker is shown above the menu title",
        callback = function(value)
            local sticker = STICKERS[value]
            if sticker and sticker.asset then
                sticker_frame.Image = sticker.asset
            end
        end,
        seperator = false,
    })
    ...
    other_section2:dropdown({
        ...
        items = {"Bubble", "None"},
        default = "Bubble",
        info = "Sound played when you click any UI button",
        callback = function(value)
            ui_sound_id = value
